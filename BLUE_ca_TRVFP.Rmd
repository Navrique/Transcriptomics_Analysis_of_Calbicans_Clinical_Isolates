...
# Raw data table
Path2Data=file.path(Data_dir,"20210611-Ca_BLUE_Raw_ReadCount.tsv")
...
RegexPattern="(?<Isolate>\\w{3}\\d{4})_(?<replicate>\\d{1})_(?<condition>\\w{2})_(?<time>\\d{1,2})"
...
for (i in ClustersID) {
  C=i
  out2 <- c(out2, knit_child(file.path("R",'20210608-STRING_Network.Rmd'), quiet=TRUE))
}
