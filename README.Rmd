...
Read processing has been performed using a homemade bioinformatics pipeline called ANAPURNA-seq. A version of the software is available on [GitHUB][1][^1] and in the [current folder][2][^2]. ANAPURNAseq (Automated Nextflow Alignment Pipeline for Unprocessed RNA-seq) perform first, various read correction step, then align corrected reads to one or two reference genomes and finally count the number of genes that can be associated with a chosen feature (often *gene* or *CDS*). For a detailed description of ANAPURNA-seq, see the [ANAPURNA-seq](#6.1-anapurna-seq) section in [Appendix](#6.-appendix).

This pipeline has been used to analyse the fastq data from the GREEN and BRAUN pair. Output of the pipeline have been added to the current folder (see the [data](Data/readcounts)). Readcount tables are then used for normalisation followed by differential gene expression analysis.
...
*Fig 4.: Log2(Fold change) matrix. The analysis focus on the comparison between the CEC3617 (LD[^LD]) and the CEC3605 (HD[^HD])*
...
## 5. GO enrichment and STRINGdb gene network
...
*fig4: source data feeding the STRINGdb database*
...
<a name="fA.1"></a>![read-procession-steps](ANAPURNA-seq/Icons/20201209-Scheme-ANAPURNA.png)

*fig A.1 : The various steps performed by ANAPURNA-seq* 

ANAPURNA-seq is designed to run on a UNIX-based system and requires [miniconda][5], [zenity][4], Bash 4.2 (or later) and [Java 8 (or later, up to 15)][6]. The computer (or sever) running ANAPURNA-seq must also be connected to the internet.
...
#### 6.1.2 Procedure to run ANAPURNA-seq 

1. Start the computer and enter my Linux session. Credentials have been furnished separately.
2. open the terminal ![Terminal](/usr/share/icons/gnome/48x48/apps/terminal.png) and mount all drives using the following command (you need to enter the password since super user's right are necessary)

```bash
MountDrives
```

3. Open the file manager and transfer fasq.gz files from the drive to a folder in the computer. Prefer to copy *fastq.gz* files in a new folder in */media/edurandau/DATA/Eric_Durandau/Sanglard/Data/Sequencing/Fastq_gz* 
4. Click the ANAPURNA-seq icon on the desktop side bar <img src="ANAPURNA-seq/Icons/ANAPURNA-seq.png" alt="ANAPURNA" style="zoom:3%;" />
5. Go to the folder where *fastq.gz* have been saved
6. Select the first genome on which reads have to be aligned
7. Select the second genome on which reads, not aligned to the first genome, have to be aligned. Following that step, a terminal window should appear and give you the progress of the processing.
8. When the analysis is done the new message should appear prompting the user to remove or not temporary files. Click OK if you are sure everything has run properly otherwise, re-run ANAPURNA with the same setting.
9. A last window should open indicating in which folder Results can be found
